    ...
    , ui(new Ui::Widget)
{
    ...
    for(;;)
    {
        tmpNode = ipcd_man_front->back_get_one(tmp_back_man, 0);
        if(tmpNode == NULL)
        {
            ipcd_man_front->back_get_one(tmp_back_man, 1);
            break;
        }
        else
        {
            QListWidgetItem *tmp = new QListWidgetItem(tmpNode->szNodeName);
